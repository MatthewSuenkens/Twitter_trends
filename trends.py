...
def extract_words(text):
    ...
    for letter in new_list:
        i += 1
        if(i == len(new_list) and letter.isalpha()):
            new_word+=letter
            words_list.append(str(new_word))
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon) - 1):
        a = 0.5 * (((latitude(i)*(longitude(i+1)) - ((latitude(i+1)*longitude(i)))
        cx = ((1/6)*A))*((latitude(i)+ latitude(i+1))*(((latitude(i)*longitude(i+1))-((latitude(i+1)*longitude(i)))
        cy = ((1/6)*A))*((longitude(i)+ longitude(i+1))*(((latitude(i)*longitude(i+1))-((latitude(i+1)*longitude(i)))
